"""
SHEET PROCESSOR v4 - DATE-AWARE DEDUP
- Detects per-row date from Raw_* tabs
- Calls dedup engine with that date so same-date matches are NOT flagged duplicate
- Paid customers always accepted (active subscribers)
"""
import gspread
import pandas as pd
...
def parse_row_date(val):
    """Try to parse a date string from a cell value. Returns date or None."""
    if not val:
        return None
    val = str(val).strip()
    if not val:
...
def process_one_tab(sh, raw_tab, is_paid=False):
    ...
    # Date column - look for "created", "date", "signed", "registered"
    ...
    for row in rows:
        ...
        # Determine the row's effective date
        row_date = TODAY  # default to today if no date column found
    ...
    log(f"   Skipped internal: {skipped_internal} | To validate: {len(emails)}")
    if not emails:
        return 0
    ...
    for raw, vres, scrape_date in zip(keep_rows, val_results, scrape_dates):
        ...
        # DATE-AWARE DEDUP CHECK
        dedup = check_lead_status(email, current_scrape_date=scrape_date)

        # For diagnostics, capture the dates this email was seen in DB
    ...
    log(f"   Wrote {len(enriched)} rows | ACCEPTED: {accepted}")
    return accepted
...
def append_daily_report(sh, metrics):
    ...
    log(f"Appended to Daily_Report: {metrics}")


def main():
    log("=" * 60)
    log("SHEET PROCESSOR v4 - DATE-AWARE DEDUP")
    log("=" * 60)
